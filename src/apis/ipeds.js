...
async function getFallEnrollmentByRace(unitid, start=_.EARLIEST, end=_.LATEST, levelOfStudy=_.ALL) {
    ...
    for (let year=start; year<=end; year++) {
        let url = new URL(`${api}/${year}/${levelOfStudy}/${endpt}?${params}`, BASE);
    ...
    return unzip(res).map(unzip).map(p => { return { label: p[0][0], data: p[1]}; });
}
